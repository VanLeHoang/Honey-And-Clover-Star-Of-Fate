...
class Chatbox:
    def __init__(self, x, y, width, height, background, font, size, color, text):
        ...
        self.text_box_surface = pygame.Surface((width, height))
        self.text_box_rect = pygame.Rect(x, y, width, height)
        self.background = pygame.transform.scale(background, self.text_box_rect.size)
    ...
    def typing(self, window):
        ...
        window.blit(self.text_box_surface, self.text_box_rect)
        self.text_box_surface.blit(self.background, self.text_box_rect)
        if self.cur_line <= num_line and self.cur_cha <= num_cha:
            ...
            for i in range(self.start_line, self.cur_line):
                ...
                self.text_box_surface.blit(line_surface, (self.text_rect.x, self.text_rect.y + self.spacing * (i - self.start_line)))
            self.text_surface = self.font.render(self.lines[self.cur_line][:self.cur_cha], True, self.color)
            self.text_box_surface.blit(self.text_surface, (self.text_rect.x, self.text_rect.y + self.spacing * (self.cur_line - self.start_line)))
            self.cur_cha += 1
    ...
    def end(self, window):
        ...
        for i in range(self.start_line, len(self.lines)):
            line_surface = self.font.render(self.lines[i], True, self.color)
            self.text_box_surface.blit(line_surface, (self.text_rect.x, self.text_rect.y + self.spacing * (i - self.start_line)))
        self.text_box_surface.blit(end_surface, end_rect)
